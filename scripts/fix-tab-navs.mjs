...
for (const file of files) {
  ...
  if (file.includes('properties-board.tsx') || file.includes('valuations-board.tsx')) {
    ...
    content = content.replace(/className="px-4 py-2 text-sm font-medium rounded-xl transition-all flex items-center gap-2 text-slate-500 hover:text-slate-900 hover:bg-slate-200\/50"/g, 
      'className="px-3.5 py-1.5 rounded-lg transition-all flex items-center gap-1.5 text-slate-500 hover:text-slate-900 hover:bg-white/45"');
  }
  ...
  content = content.replace(/className="([^"]*text-body-primary[^"]*text-slate-500[^"]*)"/g, (match, p1) => {
    const newClass = p1.replace(/\btext-body-primary\b/g, '')
                       .replace(/\bfont-semibold\b/g, '')
                       .replace(/\bfont-medium\b/g, '')
                       .replace(/\bfont-bold\b/g, '')
