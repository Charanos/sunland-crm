...
## ADR 020: Oversight Console — Consolidation, Real Maintenance Mode, and Recorded (Not Invented) System Health

The five Oversight surfaces were five separate, uneven pages: `approvals-queue-board.tsx` (528 lines), `complaints-board.tsx` (538), `support-tickets-board.tsx` (511), `reports-center-board.tsx` (220, largely static), and `/admin/system` (a pass-through into the Account console). **`Oversight Console.dc.html`** consolidates them into one hub-tabbed console with a shared dark **Oversight Pulse** tier — the same pattern ADR 018/019 established for Account & System and the Operations Scheduler.

The design also reached past what the codebase backed. Three client decisions set the scope, and each is recorded below with what "real" turned out to mean.

### 20.1 One console, five sections — and system administration relocates (revises ADR 018 §18.1)

**Decision:** `oversight-board.tsx` at the Oversight routes owns `section: approvals | complaints | tickets | reports | system`. Each of the five existing nav items keeps its own real pathname and renders the console at its section (`/admin/approvals`, `/admin/hr/complaints`, `/admin/support`, `/admin/reports`, `/admin/system`), with `/admin/oversight` as a default alias. The nav is unchanged — the five links now open one console instead of five pages.

This is the ADR 019 §19.1 routing rule applied again, and it is load-bearing rather than cosmetic: `getActiveNavItem` matches on `pathname.startsWith(href)` and pathnames drop the query string, so routing sections through `/admin/oversight?section=…` would make every Oversight nav item tie on the same prefix and break the sidebar highlight. `oversightRouteFor`/`oversightSectionForPath` in `oversight-constants.ts` hold the canonical map, and the console's URL-sync writes the pretty route on every tab switch.

**Decision — system administration now lives here, revising ADR 018 §18.1.** ADR 018 folded the old `system-admin-board` (users/roles, thresholds, audit) into the Account console's Organization scope. The design's System tab is a different thing: *operations* (maintenance mode, service health, background jobs). Rather than run two competing system-admin surfaces, the client chose one home. The Oversight Console's System section therefore absorbs the operational surface **and** the integrations + org audit log that ADR 018 had placed in the Account console's `system` org section. The Account console's Organization scope narrows to **Directory & Roles + Access Policies** — org *governance* — and its `org:system` section is removed. Because those two org sections have no nav entry of their own (they are reached through the console's scope switcher), they can safely share `/admin/account?scope=org&section=…` without any nav item tying on the prefix; `/admin/system` now belongs to Oversight.

### 20.2 Maintenance mode is a real gate, not a banner

**Decision:** `ops.maintenance_mode` / `ops.maintenance_message` live in the entity-scoped `settings` KV, and **`proxy.ts` genuinely gates the application**: with the flag on, every authenticated non-bypass role is redirected to `/maintenance`.

Three properties make this safe enough to ship:
- **The operator can never lock themselves out.** `MAINTENANCE_BYPASS_ROLES` keeps the CEO/super-admin fully inside the app, so whoever can turn it on can always reach the console to turn it off. The maintenance page itself is exempt (and is in `UNIVERSAL_PATHS`) so the redirect cannot loop, and anyone sitting on it once the flag clears is bounced back to their portal rather than left staring at a stale notice.
- **It fails open.** The proxy reads the flag through Neon's HTTP client (the app's websocket pool is not edge-compatible) inside a try/catch that returns `false` on any missing config, error or timeout. A monitoring blip must never take the whole application down.
- **It is cheap and audited.** The lookup is skipped entirely for bypass roles and cached per-isolate for 15s, so a page navigation does not pay a query; every flip writes an audit row because it is an org-wide, user-visible outage.

### 20.3 Service health is recorded, never invented

The design shows 30-day uptime bars. There is no external uptime monitor in this codebase, so rather than draw plausible bars:

**Decision:** a new **`service_health_checks`** table (no `entity_id` — infrastructure is genuinely global, ADR 003's "unless purely global configuration") stores the output of a real probe. `probeServiceHealth` genuinely round-trips the database with a timed `select 1` and checks credential presence for Ably/Upstash/M-Pesa, recording status, latency and a detail string that says *which kind* of check it was (we deliberately do not fire billable calls at M-Pesa on every console load). A service without credentials records `not_configured` — never a silent "healthy".

The bars then plot **only what this app actually recorded**. `getServiceHealthHistory` buckets by calendar day, reduces each day to its *worst* observed status (one failure must not be hidden by later successes), and returns `null` for days with no check — which the UI renders as an explicit grey gap. Uptime % is computed over measured days only and is reported alongside `measuredDays`, so the figure can be read in context. With nothing recorded yet, the pulse gauge shows "not yet measured" rather than a reassuring 100%.

### 20.4 Background jobs run for real; the absence of a scheduler is stated, not hidden

**Decision:** a small registry (`JOB_REGISTRY`) whose **Run now** invokes actual services — the health probe, a real `generatePnLReport` producing a QR-verifiable `report_exports` row, and a remittance run that drafts genuine advices through `generateRemittanceAdvice`. Each execution writes a **`job_runs`** row with status, duration, summary, error and `triggeredById`.

Two details worth keeping: the `job_runs` row is written *outside* the job's own transaction, because a failed job must leave a durable "failed" record with its error that a rollback would otherwise erase; and cadence is labelled **"manual — no scheduler configured"** rather than displaying an invented cron string. Report schedules follow the same rule: `report_schedules` stores real, editable delivery intent, the card says plainly that nothing fires on its own, and **Run now** performs the real generation and stamps `lastRunAt`.

### 20.5 Approvals, complaints and tickets — reusing what already exists

- **Bulk approve** loops the real `decideApprovalRequest` rather than issuing one wide `UPDATE`, so every per-item guard and side-effect (already-decided race check, mandate activation, maintenance cost reversion, bypass notifications) still fires. It is deliberately **not** one transaction: a single stale row must not roll back decisions that genuinely succeeded, so results come back per-item and the console reports "7 approved, 1 already decided" honestly.
- **Delegate** invents no new model. The original is closed as `escalated` (a real `approval_status` member) and a fresh pending request is created for the target tier, linked through the table's existing self-referencing **`escalatedFromId`**. The chain stays queryable and the original decision trail is never overwritten.
- **Complaints** reuse the existing service untouched; confidentiality (HR spec §6.4) stays enforced there, and the pulse exposes counts only — never a title, filer or category — so a number cannot leak case content.
- **Support tickets** gained the three things the design implied: a real **`channel`** enum captured at creation (`portal` default — honest provenance, not a guess); a real reply thread in **`support_ticket_messages`** whose first non-filer, non-internal reply stamps **`firstRespondedAt`**; and an SLA that is *derived*, not stored — `ticketSlaFor` reuses `maintenance-constants.ts`'s `slaStateFor` rather than cloning the arithmetic, differing only in that a ticket's clock stops at first response instead of resolution. Internal notes never notify the filer and are stripped from their view.

### 20.6 Honest deferrals

No cron or scheduler — report cadences and job schedules are stored intent with real manual runs, labelled as such throughout. No SMS/email/WhatsApp delivery provider (a real ticket in the seed asks for exactly this and is resolved with that explanation). No external uptime monitor — health history is what this app recorded, and gaps show as gaps. 2FA login enforcement remains deferred from ADR 018.

**Rationale & build detail:** Claude Design MCP project "Property Command Center Overhaul", file `Oversight Console.dc.html`; migration `0031`; new `oversight-constants.ts` (sections, route map, SLA, job/service registries — cross-imported by the console and both new services, the `lead-constants.ts` pattern), `services/oversight.ts` and `services/system-ops.ts`. One correction found during verification and worth recording: the first cut of the report-schedule service authorized against `finance.report.read` / `finance.report.generate`, neither of which exists in `catalog.ts` — they were replaced with the `finance.transaction.read` / `finance.transaction.write` keys `generatePnLReport` itself already enforces, and every permission key introduced in this pass was then audited against the catalog. Verified by a clean `tsc` + `eslint` and a throwaway additive-only E2E (40/40) covering the probe and its gap rendering, real job runs and unknown-job rejection, the maintenance-mode round-trip **including an explicit OFF-state regression check**, ticket channel/thread/first-response SLA and internal-note visibility, delegate-via-`escalatedFromId`, bulk-decide partial-failure reporting, schedule upsert + run-now producing a real verifiable export, and the pulse aggregates.

### 20.7 Checkpoint cleanup — dead file removal and a repo-wide lint sweep

§20.1 said the five legacy Oversight boards were "consolidated"; this pass makes that literal. `approvals-queue-board.tsx`, `complaints-board.tsx`, `support-tickets-board.tsx` and `reports-center-board.tsx` had zero remaining references anywhere in `src` (confirmed by grep before deletion) — every route that used to render them now renders `oversight-board.tsx` with a `startSection` — so the four files are deleted outright rather than left as unreachable dead code. `mandate-detail-drawer.tsx`'s `canManage` prop is dropped the same way: its only caller (`leases-board.tsx`) never passed it and the component never read it.

Beyond that, this was a non-behavioral cleanup pass across the whole `src` tree: unused imports/variables removed, one unnecessary `useMemo` dependency (`unified-market-board.tsx`), and three `<img>` → `next/image` `<Image fill>` swaps in `ledger-accounts-board.tsx` (the `images.unsplash.com` remote pattern was already whitelisted in `next.config.ts`). `src/db/seed.ts`'s unused destructured insert results are left as empty array-hole slots (`, ,`) rather than renamed, since the values were never needed — only the side-effect of the insert. Verified with a repo-wide `eslint` (0 errors, 0 warnings) and `tsc --noEmit`; the two pre-existing errors in `valuations-board.tsx` are from a concurrent, unrelated in-progress rewrite (confirmed via `git diff --stat` showing ~985 changed lines this session never touched) and are out of scope here. The "Departments" nav group (links to not-yet-built HR/Front Office/Finance sub-portal dashboards) is deliberately left as-is — it is addressed when those role dashboards are actually built.

## ADR 021: Container-Query Board Shells — the Sidebar Was Lying to Every `lg:`/`xl:` Breakpoint

The client's own screen is an XL monitor, and every board looked precisely as designed there. On a regular desktop or laptop (1280–1440px), the same pages "fell apart" — cramped KPI tiers, tables clipped instead of scrolling, rail splits collapsing at the wrong point — while mobile and tablet stayed fine. That asymmetry was the tell.

### 21.1 The root cause: `app-shell.tsx`'s sidebar offset is invisible to `lg:`/`xl:`

`AppShell` reserves `lg:pl-[272px]` for the sidebar (`lg:pl-[72px]` collapsed) and every board's own outer wrapper is `mx-auto flex max-w-[98rem] flex-col …`. A Tailwind `lg:`/`xl:` breakpoint measures the *viewport*, not that wrapper's actual content-box width — so at a common 1280–1440px laptop width the content box is only ~950–1100px even though `lg`/`xl` have long since fired, and at exactly 1024px (where `lg:` first activates and the sidebar first appears) the content box is at its *narrowest* — the precise moment a 4-column KPI tier or a `[1fr_320px]` rail split was told to switch on. Collapsing the sidebar changes the content box by 200px and no viewport breakpoint reacts to it at all. This was not a cosmetic quirk; `min-w-[900px]`/`min-w-[980px]` tables sitting inside `overflow-hidden` (not `overflow-x-auto`) wrappers meant real data was clipped and physically unreachable in that band, not merely cramped.

**Decision:** boards must react to their own content-box width, not the viewport. Tailwind v4's native `@container` support does exactly this. `globals.css` gains three named container breakpoints (`--container-board-sm: 480px`, `--container-board-md: 720px`, `--container-board-lg: 900px`, deliberately namespaced `board-` rather than overriding `--container-sm/md/lg/xl`, since that namespace already powers the widely-used `max-w-sm/md/lg/xl` scale) and a `.board-shell` class (`container-type: inline-size; container-name: board;`). Every board and detail page's existing outer wrapper takes `.board-shell`; its descendants use `@board-sm:`/`@board-md:`/`@board-lg:` instead of `sm:`/`md:`/`lg:` for anything keyed to *that board's* width — KPI tier column counts, the five-times-duplicated `[1fr_320px]` context-rail split, and fact-grids living inside that rail's narrower main column. `sm:`/`md:` elsewhere (the mobile-card ↔ desktop-table split, SKILL.md rule 9/10) are left alone — the sidebar-offset error only matters at the `lg:`/`xl:` tier, where 200–272px is a large fraction of the box.

Two small, real correctness bugs surfaced and were fixed in the same pass, unrelated to the container-query work but found while touching these files: `leases-board.tsx`'s two data tables sat inside `overflow-hidden` wrappers instead of `overflow-x-auto` (their `min-w-[900px]`/`min-w-[980px]` content was clipped, not scrollable, in the narrow band — fixed by nesting an inner `overflow-x-auto` scroll container inside the outer `overflow-hidden` that still clips the wrapper's rounded corners); and `valuations-focus-board.tsx`'s kanban root was `fixed inset-0`, which escapes `AppShell` entirely and renders *over* the sidebar rather than beside it — returned to document flow with negative margins that cancel `<main>`'s own padding for the same full-bleed feel, now correctly bounded by the sidebar column.

### 21.2 `RailLayout` — one primitive instead of five copies of the same grid

`grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-N` was hand-written, with three different gap values, in the Property, Mandate, Lease, Maintenance and Valuation full-view boards (and their loading-skeleton branches). **Decision:** `RailLayout` (`@/components/ui/erp-primitives`) owns just the grid split — `grid grid-cols-1 @board-lg:grid-cols-[1fr_320px]` — and takes a `gap` string; callers keep supplying their own two inner wrapper divs exactly as before. Maintenance's rail is a genuine outlier (380px, `sticky`) rather than the shared 320px, so it keeps its own inline `@board-lg:grid-cols-[minmax(0,1fr)_380px]` (with `sticky`/`top-4` moved to `@board-lg:` too, so it can never stick while still single-column) rather than being forced into the shared primitive.

### 21.3 Display-tier typography: two clamps, not a full fluid scale

The client asked for `clamp()`-based fluid type. Auditing `globals.css` first: the `--text-*` body/label scale (13.5px body, 12.5px caps, etc.) was already correctly sized at every width — clamping it down would only hurt mobile legibility, and 146 existing `text-[Npx]` violations (banned by SKILL.md rules 4/6, present since before this pass) needed semantic homes regardless of fluidity. **Decision:** only the display tier gets `clamp()` — `.hero-numeral` (28–42px, serif, detail-page titles) and `.kpi-numeral` (24–32px, mono, `KpiCard` and dark-tier KPI values) — replacing hardcoded escalations like `text-4xl lg:text-[42px]` and `KpiCard`'s fixed `text-[32px]`, which were themselves instances of the same sidebar-blind `lg:` bug applied to a numeral instead of a grid. The other 146 `text-[Npx]` sites were mapped to the nearest existing real token (`text-xxs`/`text-caption`/`text-sm`, etc.) rather than invented new ones. Two dead-token sites were also found and fixed in passing: `text-md` does not exist in `@theme` (SKILL.md itself listed it as valid, which is how it slipped through twice) and silently resolves to nothing.

### 21.4 A process gap caught mid-flight: the design-law doc was stale, not just the code

While auditing a concurrent frontend-agent commit (avatar system + valuations analytics cards + un-carded modal architecture — all independently design-law compliant), its brand-new Valuations KPI tier used plain `lg:grid-cols-4`. Not an oversight: `SKILL.md` rule 15 — the same document this session's own agent reads as law — still *instructed* `lg:grid-cols-4` verbatim, because it was never updated after the container-query fix landed in code. A correct fix that only lives in code and not in the doc both sides read will keep getting silently reverted by the next agent who follows the doc correctly. **Decision:** `SKILL.md` rule 15 now says `@board-lg`, and a new rule 16 records the full "why" (the sidebar-offset math, worked through in §21.1) plus the `RailLayout`/`.kpi-numeral`/`.hero-numeral` primitives, so the reasoning — not just the class name — travels with the rule. Two good patterns introduced by that same frontend-agent commit were captured as new rules 17–18 rather than left to be reinvented or reverted: the neutral-slate/real-photo avatar system (replacing yellow-background initials), and the un-carded modal architecture (a live-updating top preview bar + numbered `"01 · Section"` dividers instead of nested card wrappers per field group).

### 21.5 Honest deferrals

This pass covers CSS foundation, the shared primitives, and the full Properties/Leases/Maintenance/Valuations responsive pass (board + detail pages) — confirmed by a repo-wide `tsc --noEmit` and targeted `eslint` after every phase. **Not done in this pass, by design:** the properties-page mandate gating (main portfolio should `innerJoin` on signed/pending mandates rather than `leftJoin` showing every property regardless of mandate status, with an Intake & Archive tab for the rest); the valuation-register archive tab + a real Ready-to-Let queue for mandated, vacant-unit properties entering the sales pipeline; and a manual browser verification pass across viewport widths and both sidebar states. All three are scoped and pending, deliberately sequenced after this checkpoint so the client can review the responsive fix in isolation first.

**Rationale & build detail:** no schema or migration changes — this is a CSS/markup-only pass. Touched: `globals.css` (container tokens, `.board-shell`, `.hero-numeral`/`.kpi-numeral`), `erp-primitives.tsx` (`RailLayout`, `KpiCard` numeral), `dropdown-menu.tsx` (viewport-bottom-flip, unrelated correctness fix found in the same file), and the Properties/Leases/Maintenance/Valuations board + full-view components. Verified with `tsc --noEmit` (0 errors) and `eslint` (0 errors, 0 warnings) after each board family; no browser verification in this pass per the client's own request to review the next phase directly rather than have it pre-verified.

## ADR 022: Property Gating (Managed vs. Intake) and a Real Terminus for the Acquisition Pipeline

ADR 021 deferred two things explicitly: the Properties board showing all 132 seeded properties regardless of mandate status, and a signed valuation lingering in the register forever with no link into the sales pipeline. This ADR closes both, completing the acquisition lifecycle ADR 021 §Context originally described.

### 22.1 Managed vs. Intake — an `innerJoin`, not a client-side filter

**Decision:** `listProperties` (`properties.ts`) gains `view: "managed" | "intake"`. The mandate join it already had — `leftJoin` scoped to `status IN ('pending_approval', 'active')`, guaranteed at most one row per property by the existing partial unique index — flips to an `innerJoin` for `"managed"` (the default, finance-safe view: only properties with a real signed or in-flight mandate). `"intake"` keeps the same left join shape but adds `isNull(propertyMandates.id)`, which is the exact complement: no mandate row at all, or only a draft/terminated one. The response shape is identical between views, so `properties-board.tsx` needs only a `portfolioView` state and a `?view=` query param — no new component, no branching render logic. Verified disjoint and summing to the true count (22-assertion throwaway E2E, see §22.4).

`PortfolioHubNav`'s Leases-only "Mandates/Tenants" pill switcher is generalized to a `modeOptions` prop so Properties ("Managed"/"Intake & Archive") and Valuations (§22.2) reuse the identical control instead of three copies of the same markup with different hardcoded labels.

### 22.2 Closing the direct-upload gap

Confirmed by reading the actual code before assuming: `property-form-modal.tsx` only ever `POST`s `/api/properties`, and `createProperty` never touches `property_mandates`. Every one of the ~300 properties the client will bulk-upload directly (skipping the valuation pipeline, per the client's own framing in the original brief) would land in Intake, not Managed, with no path out.

**Decision:** on create (not edit) with an owner selected, follow the property `POST` with a real `POST /api/mandates` — `{ entityId, propertyId }` only. `createMandate` already defaults `landlordContactId` from `property.ownerContactId`, `mandateRate` from the `mandate_default_rate` setting, and `startDate` to now, so no new fields are invented; it also already runs the real GM/CEO approval-tier gate, so either `active` or `pending_approval` satisfies the Managed join without a bypass. **Deliberately not rolled back on failure** — the property is a real record regardless of whether the follow-up mandate call succeeds, consistent with `signMandateFromValuation`'s own non-transactional multi-step pattern (ADR 021's honest-deferral note on that same risk). A distinct toast reports the mandate failure separately from the property-save success. No owner selected → the property lands in Intake **by design**, stated in the success toast rather than silently gated.

### 22.3 Valuations gain a real terminus; Ready-to-Let is the honest link to the pipeline

**Decision:** `valuations.archived_at` (migration `0032`), stamped by `signMandateFromValuation` in the same final transaction that already sets `stage: "mandate_signed"` — one write, not a second pass. `listValuations` gains `view: "pipeline" | "archive"`, keyed off `isNull`/`isNotNull` on `archivedAt` rather than duplicating the stage check, so the two predicates can't drift out of sync with each other. "Pipeline" (default) includes `"declined"` — a decline can be re-opened back to `"valued"`, it is not a terminus, only `"mandate_signed"` is. The register's kanban board view has no column for `"mandate_signed"` (its `defs` array is `STAGE_ORDER + "declined"`, unchanged since before this ADR) — every Archive row would render in no column at all, so the Board toggle is hidden and `viewMode` bounces to Grid whenever Archive is selected, rather than shipping a kanban that renders as a wall of empty columns.

**Ready-to-Let** (`listReadyToLetProperties`, new) is the actual link from "a property just came under management" to the sales pipeline the client asked for — deliberately not automation. It reads real vacancy on `active`-mandate properties only (`pending_approval` is not confirmed management yet): properties with real `property_units` rows where any are `vacant`, or — for single-unit properties with no unit rows — properties whose own `status` isn't `'occupied'`. Surfaced on `pipeline-board.tsx` as a real horizontal queue with **real counts** ("N vacant units across M mandated properties", not rounded or estimated) and a "Create Lead" action per property that opens `LeadFormModal` pre-scoped to that property via a new `initialPropertyId` prop. No lead is created automatically — an agent starts a real deal from a real vacancy, which is the same "derive, don't fabricate" principle every prior ADR in this lineage has held to.

### 22.4 A pre-existing seed bug, found because this was the first reseed since ADR 020

Reseeding to generate realistic Managed/Intake/Archive/Ready-to-Let data surfaced an unrelated bug from ADR 020's Oversight Console work: `seed.ts`'s Step 1 cleanup deletes `support_tickets` before `support_ticket_messages` (added by migration `0031`), which fails on the FK the moment any ticket has a real message — nobody had reseeded since that migration landed until now. Fixed: `supportTicketMessages` deleted before its parent, and the other three Oversight tables (`jobRuns`, `reportSchedules`, `serviceHealthChecks`) added to the same cleanup step, none of which were previously cleared on reseed.

**Rationale & build detail:** migration `0032` (`valuations.archived_at` + index). New: `listReadyToLetProperties` + `GET /api/properties/ready-to-let`. Extended: `listProperties`/`listValuations` (`view` param), their API routes, `property-form-modal.tsx` (mandate fast-path), `lead-form-modal.tsx` (`initialPropertyId`), `pipeline-board.tsx` (Ready-to-Let queue), `properties-board.tsx`/`valuations-board.tsx` (mode switchers), `portfolio-hub-nav.tsx` (generalized `modeOptions`), `seed.ts` (cleanup-order fix). Verified by a clean `tsc --noEmit` + `eslint` (0/0) after every step, and a throwaway additive-only E2E (22/22) against a real dev server and reseeded data, covering: managed/intake disjoint-and-sums-to-total, direct-create-with-owner landing in Managed with a real `property_mandates` row, direct-create-without-owner landing in Intake with none, signing a real seeded valuation stamping `archivedAt` and moving it from Pipeline to Archive, and Ready-to-Let's counts matching an independently hand-checked query. Browser/visual verification is explicitly left to the client, per their own request to review this phase directly.
