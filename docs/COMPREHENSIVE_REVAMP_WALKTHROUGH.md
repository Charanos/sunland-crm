...
## 16. Oversight Console (five pages → one console)

The Approvals, Complaints, Support, Reports and System pages were consolidated into `oversight-board.tsx`. Full decision record in **ADR 020**; the reusable patterns:

- **Third application of the console pattern.** Account & System (§ADR 018), Operations Scheduler (§15) and now Oversight all share the same shell: `BoardHeader` + `Badge` eyebrow, hub tabs with real count badges, a dark pulse tier, a section divider, and one section component per tab. When a nav *group* has more than two related queues, this is now the default shape — not a new page per queue.
- **Every nav-visible section still needs its own route.** Third time this has bitten: `getActiveNavItem` matches on pathname and query strings are dropped. `oversightRouteFor`/`oversightSectionForPath` in `oversight-constants.ts` hold the map, exactly as `consoleRouteFor` does for the account console.
- **A destructive toggle needs a bypass, a fail-open, and an off-state test.** Maintenance mode genuinely gates the app in `proxy.ts`. It is safe because super-admins bypass it (the operator can always switch it back off), any read error fails open, and the E2E asserts the flag ends OFF. Apply all three whenever a switch can take a surface down.
- **Absence of data is not a green bar.** Uptime bars plot only recorded `service_health_checks`; unmeasured days render as explicit grey gaps and uptime % is reported alongside how many days were actually measured. Never let a chart imply measurement that did not happen.
- **Bulk actions loop the real single-item service.** Bulk approve calls `decideApprovalRequest` per item so every guard and side-effect still fires, deliberately *not* in one transaction, and reports per-item outcomes so partial success is visible rather than rounded up to "done".
- **Extend the existing model before inventing one.** "Delegate" needed no new columns — it reuses `approval_requests.escalatedFromId`, which already existed for escalation chains.
...
- **A `lg:`/`xl:` breakpoint measures the viewport, not the content box next to the sidebar.** `AppShell` reserves 272px (72px collapsed) for the sidebar; any board layout decision keyed to a plain `lg:`/`xl:` class is silently wrong at every width where the viewport is wide but the actual content box next to the sidebar isn't. Use `.board-shell` (on the board's existing outer wrapper) + `@board-sm:`/`@board-md:`/`@board-lg:` instead — a Tailwind v4 native `@container` query against the board's own width. `sm:`/`md:` (the mobile-card/desktop-table split) are unaffected; this only matters at the `lg:`/`xl:` tier where the sidebar offset is large.
- **`RailLayout`** (`erp-primitives.tsx`) is the one primitive for the "main content + 320px context rail" grid every full-view detail page needed — stop hand-writing `grid grid-cols-1 lg:grid-cols-[1fr_320px]` per file (it was duplicated five times with three different gap values before this pass).
- **A hardcoded numeral escalation (`text-4xl lg:text-[42px]`) is the same bug as a KPI grid.** `.hero-numeral`/`.kpi-numeral` (both `clamp()`-based) replace it. Only the display tier needs to be fluid — body/label copy was already correctly sized at every width; don't clamp what isn't broken.
- **A design-law doc that still says `lg:grid-cols-4` will keep reproducing this bug in every future PR that follows it correctly.** The fix has to land in the doc (`SKILL.md`) in the same pass it lands in code, or the next agent — including a concurrent one working the same session — will read the stale instruction and rebuild the bug from scratch. Confirmed happening in practice: a concurrent frontend-agent commit's brand-new Valuations KPI tier used plain `lg:grid-cols-4` because `SKILL.md` still said to.
