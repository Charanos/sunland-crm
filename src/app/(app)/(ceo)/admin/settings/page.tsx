...
function usePersistedSettings() {
  const [settings, setSettings] = useState<AllSettings>(() => {
    // Lazy initializer: runs once synchronously at mount.
    // Avoids setState-in-effect pattern; safe in SSR because useState
    // lazy init only runs on the client after hydration.
...
export default function SettingsPage() {
  ...
  if (!loaded) {
    return <div className="h-48 rounded-2xl skeleton-shimmer bg-slate-100 animate-pulse" />;
  }

  return (
    <div className="mx-auto max-w-[80rem] flex flex-col gap-6 pb-12 animate-fade-in">
